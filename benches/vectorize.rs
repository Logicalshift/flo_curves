...
fn criterion_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("full_distance_field_large_path_brush_parabolas", |b| b.iter(|| {
        let radius          = 32.0;
        let center          = Coord2(radius+1.0, radius+1.0);
        let circle_path     = Circle::new(center, radius).to_path::<SimpleBezierPath>();
        let circle_field    = MarchingParabolaDistanceField::from_path_region(0.0, 0.0, 65, 65, vec![circle_path]);
