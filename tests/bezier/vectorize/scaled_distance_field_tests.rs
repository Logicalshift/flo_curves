...
fn trace_half_circle_sampled() {
    ...
    let half_contour    = ScaledContour::from_contour(&circle_contour, 0.5);
...
fn trace_half_circle() {
    ...
    let half_field      = ScaledDistanceField::from_distance_field(&circle_field, 0.5);
    ...
    for curve in traced_circle[0].to_curves::<Curve<_>>() {
        for t in 0..100 {
            ...
            debug_assert!((distance - (radius/2.0)) < 1.0, "Point #{} at distance {:?}", num_points, distance);
        }
    }
...
fn trace_third_circle() {
    ...
    let circle_field    = PathDistanceField::from_path(vec![circle_path], ContourSize(1000, 1000));
    let half_field      = ScaledDistanceField::from_distance_field(&circle_field, 1.0/3.0);
    let traced_circle   = trace_paths_from_distance_field::<SimpleBezierPath>(&half_field, 0.1);
...
fn trace_circle_and_a_half() {
    ...
    let circle_field    = PathDistanceField::from_path(vec![circle_path], ContourSize(1000, 1000));
    let bigger_field    = ScaledDistanceField::from_distance_field(&circle_field, 1.5);
