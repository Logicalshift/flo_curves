...
fn check_contour_against_bitmap<TContour: SampledContour>(contour: TContour, draw_circle: bool) {
    ...
    let edges_for_y_bitmap  = bitmap_edges.iter().cloned().chunk_by(|(pos, _)| pos.1).into_iter().map(|(ypos, group)| (ypos, group.count())).collect::<HashMap<_, _>>();
    let edges_for_y_contour  = contour_edges.iter().cloned().chunk_by(|(pos, _)| pos.1).into_iter().map(|(ypos, group)| (ypos, group.count())).collect::<HashMap<_, _>>();
...
#[test]
fn circle_path_from_distance_field_2() {
    // Create a contour containing a circle in the middle, using the circular distance field
    let radius          = 300.0;
    let distance_field  = CircularDistanceField::with_radius(radius);
