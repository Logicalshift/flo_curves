...
#[test]
fn solve_t_for_close_to_start_1() {
    // -0.22216796875000006
    // No solves for curve: 5 (-0.22216796875, -0.22249345703125, -0.2228189453125, -0.22314453125)
    // Solved for curve: 7 (-0.22314453125, -0.22249348958333334, -0.22184244791666666, -0.22119140625) t=[BezierSubpathIntercept { x_pos: 0.06201171875, curve_idx: 7, t: 0.499999999999968 }]
    let t = solve_basis_for_t(-0.22216796875, -0.22249345703125, -0.2228189453125, -0.22314453125, -0.22216796875000006);
    assert!(t.len() != 0);
    assert!(t.len() == 1);
...
#[test]
fn solve_t_for_close_to_start_2() {
    // -0.22216796875000006
    // No solves for curve: 5 (-0.22216796875, -0.22249345703125, -0.2228189453125, -0.22314453125)
    // Solved for curve: 7 (-0.22314453125, -0.22249348958333334, -0.22184244791666666, -0.22119140625) t=[BezierSubpathIntercept { x_pos: 0.06201171875, curve_idx: 7, t: 0.499999999999968 }]
    let t = solve_basis_for_t(-0.22216796875, -0.22249345703125, -0.2228189453125, -0.22314453125, -0.22216796875001006);
    assert!(t.len() != 0);
    assert!(t.len() == 1);
    assert!((t[0]-0.0).abs() < 0.0001);
}
...
#[test]
fn solve_t_for_close_to_start_3() {
    // -0.22216796875000006
    // No solves for curve: 5 (-0.22216796875, -0.22249345703125, -0.2228189453125, -0.22314453125)
    // Solved for curve: 7 (-0.22314453125, -0.22249348958333334, -0.22184244791666666, -0.22119140625) t=[BezierSubpathIntercept { x_pos: 0.06201171875, curve_idx: 7, t: 0.499999999999968 }]
    let t = solve_basis_for_t(-0.22216796875, -0.22249345703125, -0.2228189453125, -0.22314453125, -0.22216796875);
    assert!(t.len() != 0);
    assert!(t.len() == 1);
    assert!((t[0]-0.0).abs() < 0.0001);
}
