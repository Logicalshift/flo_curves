...
fn deform_line_upwards() {
    ...
    let deformed    = bezier::move_point::<bezier::Curve<_>>(&curve, 0.5, &Coord2(0.0, 4.0));
...
fn deform_curve_at_halfway_point() {
    ...
    let deformed    = bezier::move_point::<bezier::Curve<_>>(&curve, 0.5, &Coord2(3.0, 4.0));
...
fn deform_curve_at_other_point() {
    let t           = 0.32;
    let curve       = bezier::Curve::from_points(Coord2(10.0, 20.0), (Coord2(0.0, 15.0), Coord2(16.0, 30.0)), Coord2(20.0, 15.0));
    let deformed    = bezier::move_point::<bezier::Curve<_>>(&curve, t, &Coord2(3.0, 4.0));
...
fn deform_curve_at_many_other_points() {
    for t in 0..100 {
        ...
        let deformed    = bezier::move_point::<bezier::Curve<_>>(&curve, t, &Coord2(6.0, -4.0));
