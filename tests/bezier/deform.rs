...
fn deform_line_upwards() {
    ...
    assert!(
        curve
            .point_at_pos(0.0)
            .distance_to(&deformed.point_at_pos(0.0))
            < 0.01
    );
    assert!(
        curve
            .point_at_pos(1.0)
            .distance_to(&deformed.point_at_pos(1.0))
            < 0.01
    );
}
...
fn deform_curve_at_halfway_point() {
    let curve = bezier::Curve::from_points(
        Coord2(10.0, 20.0),
        (Coord2(0.0, 15.0), Coord2(16.0, 30.0)),
        Coord2(20.0, 15.0),
    );
...
fn deform_curve_at_many_other_points() {
    for t in 0..100 {
        ...
        let curve = bezier::Curve::from_points(
            Coord2(5.0, 23.0),
            (Coord2(-10.0, 15.0), Coord2(26.0, 30.0)),
            Coord2(22.0, 17.0),
        );
