...
///
pub fn path_has_points_in_order<PIn>(path: PIn, expected_points: Vec<(PIn::Point, PIn::Point, PIn::Point)>, max_error: f64) -> bool
where
    PIn:        BezierPath,
    PIn::Point: std::fmt::Debug,
{
    ...
    if expected_points.len() != end_points.len() {
        assert!(false, "Number of end points differs ({} != {})", end_points.len(), expected_points.len());
        return false;
    }

    // Path must be fully closed to use this function
    let start_distance = end_points.last().unwrap().2.distance_to(&path.start_point());
    assert!(start_distance < max_error, "Path is not fully closed, start point at distance {} from end point", start_distance);
    ...
    let first_point_idx = if let Some(idx) = first_point {
        ...
        assert!(false, "Could not find first point {:?}", expected_points[0].2);
        return false;
    };
    ...
    if next_dist < prev_dist {
        ...
        for offset in 0..end_points.len() {
            ...
            let next_offset = if offset + 1 < expected_points.len() { offset + 1 } else { 0 };

            let dist_0 = expected_points[next_offset].1.distance_to(&end_points[idx].0);
            let dist_1 = expected_points[next_offset].0.distance_to(&end_points[idx].1);
            let dist_2 = expected_points[offset].2.distance_to(&end_points[idx].2);
...
fn check_points_forward_no_offset() {
    ...
    assert!(path_has_points_in_order(rectangle1, rectangle2.points().collect(), 0.1));
}
